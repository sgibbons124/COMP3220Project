...
<?php 
  ...
  $query  = "SELECT borrow.isbn, title, author, category, due_date, return_date FROM borrow, book_detail, member_detail WHERE borrow.isbn=book_detail.isbn AND borrow.member_id=member_detail.member_id AND member_detail.email='$email'";
  $result = $conn->query($query);
  if (!$result) die ("Database access failed: " . $conn->error);
  
  $rows = $result->num_rows;

  echo'<p1>Summary of Member Account</p1>';
