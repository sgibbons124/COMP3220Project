...
<?php 
  ...
  $query  = "SELECT title, author, category, first_name, count(first_name) from borrow, member, book_detail WHERE borrow.isbn=book_detail.isbn AND borrow.member_id=member.member_id group by borrow.isbn, borrow.member_id ORDER BY book_detail.title DESC";
  $result = $conn->query($query);
  if (!$result) die ("Database access failed: " . $conn->error);

  echo'<p1>Summary of Who Borrowed a Title and Number of Times Borrowed</p1>';
