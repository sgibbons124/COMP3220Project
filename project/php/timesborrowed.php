...
<?php 
  ...
  $query  = "SELECT title, author, category, first_name, count(first_name) from borrow, member, book_detail, member_detail WHERE borrow.isbn=book_detail.isbn AND borrow.member_id=member.member_id AND borrow.member_id=member_detail.member_id AND member_detail.email='$email' GROUP BY borrow.isbn, borrow.member_id ORDER BY book_detail.title DESC";
  $result = $conn->query($query);
  if (!$result) die ("Database access failed: " . $conn->error);
  
  $rows = $result->num_rows;

  echo'<p1>Number of Times Title was Borrowed</p1>';
